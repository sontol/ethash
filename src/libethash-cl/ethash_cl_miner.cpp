...
bool ethash_cl_miner::init(uint8_t const* dag, uint64_t _dagSize, unsigned workgroup_size, unsigned _platformId, unsigned _deviceId)
{
	// get all platforms
	try {
		std::vector<cl::Platform> platforms;
		...
		for (unsigned i = 0; i < 4; i++){
			...
			m_dag[i] = cl::Buffer(m_context, CL_MEM_READ_ONLY, (i == 3) ? (_dagSize - 3 * ((_dagSize >> 9) << 7)) : (_dagSize >> 9) << 7);
		}
		...
		for (unsigned i = 0; i < 4; i++)
		{
			dag_ptr[i] = m_queue.enqueueMapBuffer(m_dag[i], true, m_opencl_1_1 ? CL_MAP_WRITE : CL_MAP_WRITE_INVALIDATE_REGION, 0, (i == 3) ? (_dagSize - 3 * ((_dagSize >> 9) << 7)) : (_dagSize >> 9) << 7);
		}
		for (unsigned i = 0; i < 4; i++)
		{
			memcpy(dag_ptr[i], (char *)dag + i*((_dagSize >> 9) << 7), (i == 3) ? (_dagSize - 3 * ((_dagSize >> 9) << 7)) : (_dagSize >> 9) << 7);
			m_queue.enqueueUnmapMemObject(m_dag[i], dag_ptr[i]);
		}
		// compute dag on CPU
		/*{
...
void ethash_cl_miner::hash(uint8_t* ret, uint8_t const* header, uint64_t nonce, unsigned count)
{
	...
	*/
	...
	m_hash_kernel.setArg(2, m_dag[0]);
	m_hash_kernel.setArg(3, m_dag[1]);
	m_hash_kernel.setArg(4, m_dag[2]);
	m_hash_kernel.setArg(5, m_dag[3]);
...
void ethash_cl_miner::search(uint8_t const* header, uint64_t target, search_hook& hook)
{
	try {
		...
		*/
		...
		m_search_kernel.setArg(2, m_dag[0]);
		m_search_kernel.setArg(3, m_dag[1]);
		m_search_kernel.setArg(4, m_dag[2]);
		m_search_kernel.setArg(5, m_dag[3]);
